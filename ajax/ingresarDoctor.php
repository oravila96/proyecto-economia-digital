<?php
    ...
    switch ($_GET["accion"]) {
        case '3':
            ...
            if(isset($_POST["int_identidad"])){
                $identidad = $_POST["int_identidad"];
            }
            if(isset($_POST["txt_colegiacion"])){
                $colegiacion = $_POST["txt_colegiacion"];
            }
            ...
            if($colegiacion=="" || $colegiacion==NULL){
                echo "Ingrese la colegiacion";
            }
            ...
            if($telefono=="" || $telefono==NULL){
                ...
                $unir=oci_parse($conn,'DECLARE
                                        ...
                                        BEGIN
                                            SP_AGREGAR_DOCTOR(:p1, :p2, :p3, :p4, :p5, :p6, :p7, :p8, :p9, :p10, :p11, :p12);
                                        END;');
                ...
                oci_bind_by_name($unir, ':p8', $colegiacion);
                oci_bind_by_name($unir, ':p9', $telefono);
                oci_bind_by_name($unir, ':p10', $fecha);
                oci_bind_by_name($unir, ':p11', $codigoError, 40);
                oci_bind_by_name($unir, ':p12', $mensajeError, 40);
                oci_execute($unir);
